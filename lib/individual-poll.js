...
function prepare(a) {
    ...
    for (const cfg of configuredDevices) {
        ...
        // its normal initial poll for every configured device. The "0" printed
        // by the upstream initialization log therefore means only that its
        // internal timer is disabled; this module owns the real runtime timer.
        cfg.updateInterval = 0;
    }

    if (a.log) {
        a.log.info('Runtime poll interval switch enabled: default=individual; standard=' + standardInterval + 's; ' +
            devices.size + ' device(s). Upstream recurring timers are internally disabled (0).');
...
function scheduleNext(deviceId) {
    ...
    timers.set(device.id, setTimeout(() => {
        ...
        // Re-arm as a safety net even if updateNow happens to be ignored because
        // the same device is already queued. A successful read will replace this
        // timer and schedule cleanly from the end of that read.
...
async function ensureControlState(device) {
    ...
    // 2.7.5 created this state with default=false. Migrate that old object once
    // to the new requested default=true. The marker prevents later restarts from
