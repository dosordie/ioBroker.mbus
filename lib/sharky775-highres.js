...
async function applicationReset(s, target, timeout) {
    // Mirrors SHARKY 775 R4 ReadConfigurationOperation.readTelegrammByApplicationReset(16):
    // applicationlayer.write([0x10], 0x50), followed by applicationlayer.read().
    // On M-Bus that is SND_UD/CI=0x50/data=0x10, ACK, then REQ_UD2 and consume RSP_UD.
    ...
    const request = shortFrame(0x5B, target);
    debugFrame('TX application reset REQ_UD2', request);
    const response = await exchange(s, request, r => r[0] === 0x68, timeout);
    debugFrame('RX application reset data', response);
    return response;
}
