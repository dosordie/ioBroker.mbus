...
function highResIntervalSeconds(cfg) {
    if (!cfg || cfg.sharkyHighResInterval === '' || cfg.sharkyHighResInterval === undefined || cfg.sharkyHighResInterval === null) return 0;
    const value = Number(cfg.sharkyHighResInterval);
    return Number.isFinite(value) && value > 0 ? value : 0;
}

function highResDue(id, cfg) {
    const interval = highResIntervalSeconds(cfg);
    if (interval <= 0) return true;
    const last = lastHighResRead.get(String(id));
    return !last || (Date.now() - last) >= interval * 1000;
}
...
async function readCycle(a, pending) {
    ...
    const interval = highResIntervalSeconds(cfg);
    if (!highResDue(pending.id, cfg)) {
        if (a.log) {
            const last = lastHighResRead.get(pending.id) || 0;
            const remaining = Math.max(1, Math.ceil((interval * 1000 - (Date.now() - last)) / 1000));
            a.log.debug('SHARKY 775 ' + ns + ' high-resolution read skipped; next due in about ' + remaining + 's');
        }
        return;
    }

    const enabledId = ns + '.data.' + p.output.enabledState;
    if (!(await isEnabled(a, enabledId))) return;
    ...
    let target;
    try {
        target = await address(s, pending.id, timeout);
        let sc = scaling.get(ns);
        if (!sc) {
            sc = await readScaling(s, target, timeout);
            scaling.set(ns, sc);
            a.log && a.log.debug('SHARKY 775 ' + ns + ' scaling: ZCEINH=' + sc.zceinh + ', KOMMAE=' + sc.kommae + ', ENAKCM=' + sc.enakcm);
        }
        ...
        lastHighResRead.set(pending.id, Date.now());
        a.log && a.log.debug('SHARKY 775 ' + ns + ' high-resolution energy: ' + wh + ' Wh' +
            (interval > 0 ? ' (interval ' + interval + 's)' : ' (every device poll)'));
    } finally {
        if (target !== undefined) {
            try {
                await applicationReset(s, target, timeout);
            } catch (e) {
                if (a.log) a.log.debug('SHARKY 775 application reset cleanup failed for ' + ns + ': ' + e.message);
            }
        }
        s.destroy();
    }
...
function energyWh(bytes, sc) {
    const raw = decodeBcd(bytes);
    const highResCount = raw / 100000n;
    const value = Number(highResCount) * Math.pow(10, -3 - sc.kommae);
...
async function address(s, id, timeout) {
    ...
    debugFrame('TX select ' + id, f);
    const ack = await exchange(s, f, r => r.length === 1 && r[0] === 0xE5, timeout);
    debugFrame('RX select ACK ' + id, ack);
    return 0xFD;
}
...
async function readMem(s, target, addr, len, timeout) {
    ...
    const command = longFrame(0x53, target, 0x51, payload);
    debugFrame('TX RAM read addr=' + addr + ' len=' + len, command);
    const ack = await exchange(s, command, r => r.length === 1 && r[0] === 0xE5, timeout);
    debugFrame('RX RAM ACK addr=' + addr, ack);

    const request = shortFrame(0x5B, target);
    debugFrame('TX REQ_UD2 addr=' + addr, request);
    const response = await exchange(s, request, r => r[0] === 0x68, timeout);
    debugFrame('RX RAM data addr=' + addr, response);
    return parseMem(response, addr, len);
}

async function applicationReset(s, target, timeout) {
    // SHARKY 775 R4 application reset: SND_UD/CI=0x50/data=0x10.
    // Real R4 meters acknowledge this command with E5 and do not queue a
    // separate RSP_UD telegram, so the ACK completes the cleanup transaction.
    const command = longFrame(0x53, target, 0x50, Buffer.from([0x10]));
    debugFrame('TX application reset', command);
    const ack = await exchange(s, command, r => r.length === 1 && r[0] === 0xE5, timeout);
    debugFrame('RX application reset ACK', ack);
    return ack;
}
...
function parseMem(f, addr, len) {
    ...
    for (let i = 19; i + 5 <= end; i++) {
        ...
        // SHARKY response format observed on R4 / facelift:
        // 0F 04 <selector> <address low> <address high> <requested bytes...>
        // The selector byte is NOT a payload length. Examples from real meters:
        //   0F 04 02 B0 10 68                         (addr 4272, len 1)
        //   0F 04 03 0C 02 60 89 50 49 60 80 02 00 (addr 524, len 8)
        const selector = f[i + 2];
        const a = f[i + 3] | (f[i + 4] << 8);
        const start = i + 5;
        if (a !== addr) continue;
        if (start + len > end) throw new Error('truncated SHARKY response');
        const value = f.slice(start, start + len);
        if (adapter && adapter.log) {
            adapter.log.debug('SHARKY 775 parsed RAM addr=' + addr + ' selector=0x' + selector.toString(16).padStart(2, '0').toUpperCase() + ' value=' + value.toString('hex').toUpperCase());
        }
        return value;
    }
...
function shortFrame(c, a) {
    return Buffer.from([0x10, c, a, (c + a) & 255, 0x16]);
}
...
function debugFrame(label, frame) {
    if (adapter && adapter.log) adapter.log.debug('SHARKY 775 ' + label + ': ' + Buffer.from(frame).toString('hex').toUpperCase());
}
