...
function verify(f) {
    ...
    return f[0] === 0x68 && f.length >= 9 && f[1] === f[2] && f[3] === 0x68 && f.length === f[1] + 6 && f.at(-1) === 0x16 && sum(f.slice(4, -2)) === f.at(-2);
}
...
function logError(a, id, e) {
    ...
    if (a.log) (n === 1 ? a.log.info : a.log.debug)(msg + (n === 1 ? ' (standard M-Bus values are unaffected)' : ''));
}
