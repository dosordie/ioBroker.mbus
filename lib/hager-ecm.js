...
function install() {
    ...
    MbusMaster.prototype.getData = function (id, cb) {
        return oldGetData.call(this, id, (err, data) => {
            cb(err, data);
            if (!err && data && adapter) {
                publish(adapter, data).catch(e => {
                    if (adapter && adapter.log) adapter.log.debug('Hager ECM decode failed for M-Bus ID ' + id + ': ' + e.message);
                });
            }
        });
    };
...
module.exports = {install, decode, publish, parameterSetBytes};
